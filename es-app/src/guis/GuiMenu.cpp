...
void GuiMenu::openScraperSettings()
{
	...
	for(auto it = scrapers.cbegin(); it != scrapers.cend(); it++)
		scraper_list->add(_(it->c_str()), *it, *it == Settings::getInstance()->getString("Scraper"));
...
void GuiMenu::openUISettings()
{
	...
	s->addSaveFunc([ UImodeSelection, window]
	{
		...
		if (selectedMode != "Full")
		{
			...
			msg << boost::locale::format(_("You are changing the UI to a restricted mode:\n{1}\n"	\
				"This will hide most menu-options to prevent changes to the system.\n"	\
				"To unlock and return to the full UI, enter this code: \n"		\
				"\"{2}\"\n\n"								\
				"Do you want to proceed?"))
			  % _(selectedMode.c_str()) % UIModeController::getInstance()->getFormattedPassKeyStr();
			window->pushGui(new GuiMsgBox(window, msg.str(),
				_("YES") , [selectedMode] {
