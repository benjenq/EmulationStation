	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	const char* msg2str = firstRun ? N_("PRESS F4 TO QUIT AT ANY TIME." ) : N_("PRESS ESC TO CANCEL.");
	mMsg2 = std::make_shared<TextComponent>(mWindow, _(msg2str).c_str(), Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
